...
export function Navbar() {
  ...
  return (
    <nav className="w-full bg-white flex justify-between items-center xl:py-5 py-3  fixed top-0 left-0 z-20 shadow-xs">
      ...
      {menu ? (
        <ul className="flex flex-col h-screen bg-white z-100 w-96 fixed top-0 left-0 px-6 font-semibold py-2 text-sm">
          <div className=" border-b-1 border-gray-200 py-4 flex justify-between items-center">
            ...
            />
            <button
              className="cursor-pointer hover:bg-teal-600 hover:text-white rounded-full border p-1 text-gray-700 "
        ...
        <div className="xl:hidden block absolute transition ">
          <button className="cursor-pointer px-4" onClick={() => setMenu(true)}>
            {" "}
            <ChartNoAxesGantt className="w-8 h-8 text-gray-600  " />
          </button>
        </div>
      )}
      ...
      <div className="flex justify-around items-center mx-6 gap-8 ">
        ...
        <div className="flex gap-4  items-center justify-center">
          {open ? (
            ...
            <div className="fixed top-0 left-0  w-full bg-white-40 backdrop-blur-lg  py-4 flex justify-center items-center gap-5 z-50 border border-b-teal-500 ">
              <div className="w-[50%] flex place-items-center">
                <button className="absolute cursor-pointer text-gray-500  p-3 text-sm ">
          ...
          {isLoggedin ? (
            <button
            aria-label="Logout"
              className="bg-teal-800 py-3 px-4 rounded-sm font-semibold text-white text-sm cursor-pointer"
