    ...
    <div className="fixed top-60 right-0 z-50"> 
      <Sheet>
        <SheetTrigger className="bg-teal-700 p-5 rounded-l-sm font-semibold text-white text-sm cursor-pointer flex flex-col ">
           ...
           <br className="my-4"/>
           <span className="bg-white text-teal-700 rounded-sm px-6  py-2  cursor-pointer"> ${cart?.data?.totalPrice}</span>
        </SheetTrigger>
        <SheetContent className="w-[90vw] sm:w-[400px] overflow-y-auto">
          ...
          <div className=" px-5 w-full bottom-10 absolute">
            <button className="w-full py-3 bg-teal-700 text-white rounded-full flex justify-between items-center px-6 font-medium">
              Checkout
              <span className="bg-white text-teal-700 px-3 py-1 rounded-full">
